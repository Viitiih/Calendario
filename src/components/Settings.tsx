  ...
  const handleToggleNotifications = async () => {
    ...
    if (permission === "granted") {
      ...
      new Notification("Notificações ativadas", {
        body: "Você receberá avisos importantes do calendário.",
      });
    } else {
      onUpdateUser({ notificationsEnabled: false });
    ...
    <div className="space-y-6 sm:space-y-8 pb-10 overflow-x-hidden">
      <div className="text-center space-y-3">
        <div className="flex items-center justify-center gap-2 mb-2 px-2">
           ...
           <span className="tech-label tracking-[0.22em] sm:tracking-[0.4em] opacity-40 text-center">{t('settings')}</span>
           <div className="w-8 h-[2px] rounded-full bg-blue-500/20" />
        </div>
        <h2 className={cn(
          "text-2xl min-[390px]:text-3xl sm:text-4xl font-display font-black tracking-tighter leading-tight px-2",
          isDarkMode ? "text-slate-100" : "text-slate-900"
        )}>{t('personalize_experience')}</h2>
      </div>

      <div className="grid grid-cols-3 bg-slate-500/5 p-1.5 rounded-[28px] sm:rounded-[32px] gap-1 w-full max-w-[340px] mx-auto">
        {[
          ...
          <button
            ...
            className={cn(
              "min-w-0 py-3 px-1 rounded-[20px] sm:rounded-[24px] flex flex-col items-center gap-1 transition-all duration-500 relative overflow-hidden group",
              activeSubTab === st.id 
          ...
          >
            ...
            <span className="max-w-full truncate text-[8px] min-[390px]:text-[9px] font-black uppercase tracking-[0.08em] sm:tracking-widest relative z-10">{st.label}</span>
          </button>
        ))}
      </div>
        ...
        <AnimatePresence mode="wait">
          {activeSubTab === 'profile' && (
            ...
            >
              {/* User Profile Card */}
        <div className={cn(
          "p-5 min-[390px]:p-6 sm:p-8 rounded-[28px] sm:rounded-[40px] border transition-all duration-300 relative overflow-hidden",
          isDarkMode 
          ...
          <div className="flex items-center gap-4 sm:gap-5 mb-6 sm:mb-8 relative z-10 min-w-0">
             <div className={cn(
              "w-14 h-14 sm:w-16 sm:h-16 shrink-0 rounded-[22px] sm:rounded-[24px] flex items-center justify-center text-white font-tech font-black text-2xl sm:text-3xl shadow-2xl transition-all duration-300 hover:scale-110",
                    ...
                    <div className="flex items-center gap-4 min-w-0">
                      <div className={cn(
                         ...
                         user.notificationsEnabled
                           ...
                           : isDarkMode ? "bg-white/[0.05] text-slate-400" : "bg-white text-slate-500 shadow-sm"
                      )}>
                        {user.notificationsEnabled ? <Bell size={20} strokeWidth={2} /> : <BellOff size={20} strokeWidth={2} />}
                      </div>
                      <div className="min-w-0">
                        <span className={cn("text-sm font-display font-black block", isDarkMode ? "text-slate-300" : "text-slate-700")}>Notificações</span>
                        <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                          {notificationPermission === "denied" && !user.notificationsEnabled
                            ? "Bloqueadas no navegador"
                            : user.notificationsEnabled ? "Ativas" : "Toque para ativar"}
                        </span>
                      </div>
                    </div>
                    <button 
                      ...
                      className={cn(
                        "w-14 h-8 rounded-full transition-all duration-300 relative inner-shadow shrink-0",
                        user.notificationsEnabled ? "bg-emerald-500" : "bg-slate-200"
                      )}
                    >
                      <div className={cn(
                        "absolute top-1 w-6 h-6 rounded-full bg-white transition-all duration-300 shadow-xl flex items-center justify-center",
                        user.notificationsEnabled ? "left-7" : "left-1"
                         ...
                         {user.notificationsEnabled ? <Bell size={10} className="text-emerald-600" /> : <BellOff size={10} className="text-slate-400" />}
                      </div>
                    </button>
                  </div>
