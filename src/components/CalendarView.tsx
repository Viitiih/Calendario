...
import { CalendarStatsSkeleton, CalendarGridSkeleton, MembersSkeleton } from "./SkeletonLoader";
...
export const getSplitGradient = (workDays: any[], calendarUsers: any[] = []) => {
  ...
  const colors = [...new Set(workDays.map(wd => {
    ...
    return calendarUser ? calendarUser.color : (wd.userId?.startsWith?.('#') ? wd.userId : "#3B82F6");
...
export const CalendarView = memo(({ 
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        <CalendarStatsSkeleton isDarkMode={isDarkMode} />
      ) : (
      ...
      >
        ...
        <div className="grid grid-cols-7 gap-y-1.5 gap-x-1 sm:gap-y-3 sm:gap-x-1.5 mb-2 relative z-10 calendar-grid">
          {days.map(day => {
            ...
            return (
              <button
                ...
                style={isOutOfMonth ? { opacity: 0.1 } : undefined}
                className={cn(
                  "relative aspect-square rounded-2xl transition-colors duration-150 group calendar-day-btn",
                  ...
                  isEraseMode && isWorkDay ? "ring-2 ring-rose-500 ring-offset-2 ring-offset-transparent shadow-[0_0_15px_rgba(244,63,94,0.3)]" : ""
                )}
              >
                <div className={cn(
                  "absolute inset-0 rounded-2xl transition-opacity duration-150 opacity-0 group-hover:opacity-100",
                ...
                <div className="absolute bottom-1 left-1 flex flex-wrap gap-1 z-10 max-w-[90%] pointer-events-none">
                    ...
                    {calendarMode === 'work' && data.study.map((wd, idx) => (
                        <div 
                          key={`study-${wd.id || idx}`}
                          className="w-2 h-2 rounded-full bg-blue-400 border border-blue-300/30"
                        />
                      ))}
                    ...
                    {calendarMode === 'expenses' && data.expenses.map((e, idx) => (
                        <div 
                          key={`exp-${e.id || idx}`}
                          className="w-1.5 h-1.5 rounded-[1px] border border-white/10 rotate-45"
                          style={{ 
                ...
                {calendarMode === 'work' && isWorkDay && data.work[0]?.startTime && data.work[0]?.endTime && (
                    <div className="absolute top-1 right-2 text-right pointer-events-none z-10">
      ...
      <div className="flex flex-wrap items-center gap-2 px-2 pb-10 fade-in">
