...
export const LoginScreen = memo(({
  ...
  const [authMode, setAuthMode] = useState<
    "initial" | "invite" | "login" | "register" | "phone"
  >("initial");
  ...
  // Cleanup recaptcha when leaving phone mode
  useEffect(() => {
    if (authMode !== "phone") {
      try {
  ...
  const handleAppleLogin = async () => {
    ...
    try {
      ...
      onLogin(
        firebaseUser.displayName || firebaseUser.email || "Usuário Apple",
        userColor,
        firebaseUser.uid
      );
    } catch (err: any) {
  ...
  const handleSendOtp = async () => {
    ...
    if (!cleaned || cleaned.length < 8) {
      setError("Digite um número de telefone válido com código do país (ex: +5511999999999).");
      return;
    }
    setLoading(true);
    setError(null);
    try {
      // Setup recaptcha invisível no container
      const verifier = setupRecaptcha("recaptcha-container");
      recaptchaVerifierRef.current = verifier;
      const result = await sendPhoneOtp(cleaned, verifier);
      setConfirmationResult(result);
      setPhoneStep("otp");
    } catch (err: any) {
      console.error("Phone OTP error:", err);
      // Limpa recaptcha em caso de erro para poder tentar de novo
      try {
        recaptchaVerifierRef.current?.clear();
        recaptchaVerifierRef.current = null;
      } catch {}
      if (err.code === "auth/invalid-phone-number") {
        setError("Número de telefone inválido. Use o formato +5511999999999.");
  ...
  const handleConfirmOtp = async () => {
    if (!otp || otp.length < 4 || !confirmationResult) {
    ...
    setError(null);
    try {
      const firebaseUser = await confirmPhoneOtp(confirmationResult, otp);
      const savedUser = localStorage.getItem("worksync_user");
      let userColor = USER_COLORS[0];
      if (savedUser) {
        try {
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(160deg, #0a0a14 0%, #080810 50%, #0a0812 100%)",
      }}
    >
      {/* reCAPTCHA invisível — deve existir no DOM durante o fluxo de telefone */}
      ...
      <div className="flex flex-col items-center pt-16 pb-8 px-6">
        <div
          className="w-16 h-16 rounded-[22px] flex items-center justify-center mb-5 relative"
          style={{
            background: "linear-gradient(135deg, #6366f125, #6366f108)",
      ...
      <div className="flex-1 px-5 pb-8 flex flex-col">
        <div
          className="rounded-[28px] p-6 flex-1 flex flex-col"
        ...
        >
          <AnimatePresence mode="wait">
            ...
            {authMode === "initial" && (
              ...
              >
                <p className="text-[10px] font-black tracking-[0.25em] text-slate-600 uppercase text-center mb-1">
                  Escolha como entrar
                </p>
                ...
                <button
                  ...
                  className="w-full flex items-center gap-4 px-5 py-4 rounded-2xl font-bold text-[15px] text-white transition-all active:scale-[0.98] disabled:opacity-50"
                  style={{
                    background: "#ffffff08",
                    border: "1px solid #ffffff0f",
                  }}
              ...
              >
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h2 className="text-xl font-black text-white">
                    {phoneStep === "number" ? "Entrar com Telefone" : "Verificar Código"}
                  </h2>
                </div>
                ...
                {phoneStep === "number" ? (
                  <>
                    <p className="text-slate-500 text-sm">
                      Digite seu número com o código do país. Você receberá um
                      SMS com um código de verificação.
                    </p>
                    <div className="relative">
                      <input
                        type="tel"
                  ...
                  <>
                    ...
                    <input
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" && handleConfirmOtp()
                      }
                      className="w-full px-5 py-5 rounded-2xl text-white font-black text-center text-3xl tracking-[0.5em] focus:outline-none placeholder:text-slate-700 placeholder:tracking-normal placeholder:font-medium placeholder:text-base"
                      style={{
                        background: "#ffffff08",
                        border: "1px solid #6366f130",
                      }}
                    ...
                    >
                      Não recebi o código —{" "}
                      <span className="text-indigo-400">Reenviar</span>
                    </button>
                  </>
                )}
              </motion.div>
