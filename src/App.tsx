...
import { CalendarDays, DollarSign, TrendingUp, Share2, WifiOff } from "lucide-react";
  ...
  return (
    <div className={cn("min-h-screen flex flex-col transition-all duration-500 pb-24 selection:bg-blue-500/30 overflow-x-hidden relative", isDarkMode ? "bg-[#050505] text-white selection:text-blue-200" : "bg-[#FAFAFA] text-slate-900 selection:text-blue-900")}>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <motion.div
          className={cn("absolute -top-[20%] -left-[10%] w-[70%] h-[70%] rounded-full blur-[120px] opacity-20 transition-colors duration-1000", isDarkMode ? "bg-blue-900/40" : "bg-blue-100/60")}
          animate={{ x: [0, 18, -8, 0], y: [0, 10, 20, 0], scale: [1, 1.06, 0.98, 1] }}
          transition={{ duration: 14, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className={cn("absolute -bottom-[10%] -right-[5%] w-[60%] h-[60%] rounded-full blur-[100px] opacity-10 transition-colors duration-1000", isDarkMode ? "bg-purple-900/30" : "bg-purple-100/40")}
          animate={{ x: [0, -14, 10, 0], y: [0, -18, -8, 0], scale: [1, 0.96, 1.08, 1] }}
      ...
      <main ref={mainRef} className="flex-1 w-full max-w-[500px] mx-auto px-4 sm:px-6 relative z-10 pb-[120px] min-h-[100dvh]">
        <AnimatePresence mode="wait" custom={direction}>
          <motion.div
            key={activeTab}
            custom={direction}
            initial={{ opacity: 0, x: direction * 28, scale: 0.985, filter: "blur(8px)" }}
            animate={{ opacity: 1, x: 0, scale: 1, filter: "blur(0px)" }}
            exit={{ opacity: 0, x: direction * -28, scale: 0.985, filter: "blur(8px)" }}
      ...
      <nav className={cn("fixed bottom-6 left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] max-w-[480px] h-[76px] px-6 flex justify-around items-center z-[100] transition-all duration-300 backdrop-blur-3xl rounded-[32px] border", isDarkMode ? "bg-[#090909]/80 border-white/[0.05] shadow-[0_20px_40px_rgba(0,0,0,0.6)]" : "bg-white/80 border-slate-200/60 shadow-[0_20px_40px_rgba(0,0,0,0.08)]", isModalOpen && "hidden")}>
        {[
          ...
          return (
            <motion.button key={tab.id} whileTap={{ scale: 0.88 }} whileHover={{ y: -2 }} onClick={() => handleTabChange(tab.id as any)} className={cn("relative flex flex-col items-center justify-center gap-1 group transition-all duration-200", isActive ? "scale-110" : "opacity-40 hover:opacity-100")}>
