// src/App.tsx — Versão Firebase
// Principais mudanças vs versão original:
//   • socket.emit("update-calendar") → updateCalendar() do calendarService
//   • socket.on("calendar-updated") → onSnapshot via subscribeToCalendar()
//   • fetch("/api/calendar/...") → getCalendar() / getCalendarByInviteCode()
//   • VITE_BACKEND_URL / VITE_STATIC_MODE → removidos (não são mais necessários)
...
export default function App() {
  ...
  // ─── Usuário (persiste no localStorage como antes) ─────────────────────────
  ...
  // ─── Subscrição em tempo real (substitui socket.on("calendar-updated")) ────
  useEffect(() => {
    if (!calendarId) return;

    // Tenta carregar o calendário; cria se não existir
    const initCalendar = async () => {
      ...
      try {
        ...
        if (!data) {
          // Primeiro acesso: cria o calendário no Firestore
          if (user) {
            data = await createCalendar(calendarId, user.id, user.name, user.color);
          } else {
            // Usuário ainda não fez login, aguarda
            return;
          }
        }
    ...
    // Listener em tempo real — substitui socket.on("calendar-updated")
    // O Firestore propaga automaticamente para todos os dispositivos conectados
    const unsubscribe = subscribeToCalendar(calendarId, (data) => {
      setCalendarData(data);
    });

    return () => {
      unsubscribe(); // substitui socket.off("calendar-updated")
    };
  }, [calendarId, user?.id]);

  // ─── Atualizar calendário (substitui socket.emit("update-calendar")) ────────
  const updateCalendar = useCallback(
    async (newData: CalendarData) => {
      // Atualiza localmente de imediato (optimistic update)
      setCalendarData(newData);
      // Persiste no Firestore — o onSnapshot vai propagar para os outros usuários
      await fbUpdateCalendar(newData);
    },
    []
  );

  // ─── Convite (substitui fetch("/api/calendar/:code")) ─────────────────────
  ...
  useEffect(() => {
    ...
    if (!isUserMember && !isUserPending) {
      if (!calendarData.ownerId) {
        ...
        // Usuário já existe → vai para pendentes via Firestore
        joinCalendarAsPending(calendarId, {
          id: user.id,
          name: user.name,
          color: user.color,
        });
  ...
  const handleLogin = useCallback(
    (name: string, color: string) => {
      ...
      // Adiciona usuário ao calendário se ainda não for membro
  ...
  const isPending = useMemo(
    ...
    [calendarData.pendingUsers, user?.id]
  );
  ...
  // ─── UI principal (idêntica à original, sem nenhuma mudança visual) ────────
  return (
    <div
      className={cn(
    ...
    >
      ...
      <main
        ref={mainRef}
        className="flex-1 w-full max-w-[500px] mx-auto px-4 sm:px-6 relative z-10 pb-[120px] min-h-[100dvh]"
      >
        <AnimatePresence mode="wait" custom={direction}>
          <motion.div
            ...
            onDragEnd={(_, info) => {
              const threshold = 50;
              const allTabs: typeof activeTab[] = [
                "calendar", "finance", "goals", "share", "settings",
              ];
              const available = allTabs.filter((t) => t !== "goals" || user.showGoals);
              const currentIndex = available.indexOf(activeTab as any);
              if (info.offset.x > threshold && currentIndex > 0) {
                handleTabChange(available[currentIndex - 1]);
              } else if (
      ...
      >
        {[
          ...
          ...(user?.showGoals
            ? [{ id: "goals", icon: TrendingUp, label: t("meta_financeira") }]
            : []),
          ...(isMember
      ...
      <AnimatePresence>
        {isModalOpen && selectedDate && (
          <Suspense
            fallback={
              <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/80 text-white">
                Carregando...
              </div>
            }
          >
            <DayModal
              ...
              onSave={(workDays, expenses, incomes = []) => {
                const affectedWorkDates = workDays.map((wd) =>
                  format(parseISO(wd.date), "yyyy-MM-dd")
                );

                const updatedWorkDays = [...(calendarData.workDays || [])].filter(
                  (wd) => {
                    ...
                    if (isAffected && wd.userId === user.id) {
                      const hasSameType = workDays.some(
                        (nwd) =>
                          format(parseISO(nwd.date), "yyyy-MM-dd") === wdDateStr &&
                          (nwd.type === wd.type ||
                            (!nwd.type && wd.type === "work") ||
                            (!wd.type && nwd.type === "work"))
                      );
                      return !hasSameType;
                    }
                    return true;
                ...
                const updatedExpenses = [...(calendarData.expenses || [])].filter(
                  (e) => !isSameDay(parseISO(e.date), selectedDate)
                );
                const updatedIncomes = [...(calendarData.incomes || [])].filter(
                  (i) => !isSameDay(parseISO(i.date), selectedDate)
                );
