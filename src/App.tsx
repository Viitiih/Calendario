    ...
    <div className={cn("min-h-screen flex flex-col transition-all duration-500 pb-24 selection:bg-blue-500/30 overflow-x-hidden relative", isDarkMode ? "bg-[#050505] text-white selection:text-blue-200" : "bg-[#FAFAFA] text-slate-900 selection:text-blue-900")}>
      ...
      <nav className={cn("fixed bottom-[max(0.75rem,env(safe-area-inset-bottom))] sm:bottom-6 left-1/2 -translate-x-1/2 w-[calc(100%-0.75rem)] sm:w-[calc(100%-2rem)] max-w-[480px] h-[68px] sm:h-[76px] px-2 min-[390px]:px-3 sm:px-6 flex justify-around items-center z-[100] transition-all duration-300 backdrop-blur-3xl rounded-[28px] sm:rounded-[32px] border", isDarkMode ? "bg-[#090909]/80 border-white/[0.05] shadow-[0_20px_40px_rgba(0,0,0,0.6)]" : "bg-white/80 border-slate-200/60 shadow-[0_20px_40px_rgba(0,0,0,0.08)]", isModalOpen && "hidden")}>
        {[
          ...
          return (
            <motion.button key={tab.id} whileTap={{ scale: 0.88 }} whileHover={{ y: -2 }} onClick={() => handleTabChange(tab.id as any)} className={cn("relative flex min-w-0 flex-1 flex-col items-center justify-center gap-0.5 sm:gap-1 group transition-all duration-200", isActive ? "scale-110" : "opacity-40 hover:opacity-100")}>
